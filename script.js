...
// Mobile-safe viewport sizing. CSS handles the sizing; this only
// updates the real viewport height for browsers with dynamic toolbars.
function updateOpeningViewport(){
  const loader = document.getElementById("loader");
...
updateOpeningViewport();
window.addEventListener("resize", updateOpeningViewport, {passive:true});
window.addEventListener("orientationchange", () => setTimeout(updateOpeningViewport, 150), {passive:true});
if(window.visualViewport){
  window.visualViewport.addEventListener("resize", updateOpeningViewport, {passive:true});
}
